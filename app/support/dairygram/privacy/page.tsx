...
const TRANSLATIONS: Record<Language, {
  ...
  ja: {
    ...
    sections: {
      intro: {
        title: "1. はじめに",
        content: "Dairygram（以下「本アプリ」）は、写真・気分・天気を添えて1日1つの日記を記録できるフォト日記アプリです。本プライバシーポリシーは、本アプリをご利用いただく際の情報の取り扱いについて説明します。",
      ...
      dataCollection: {
        ...
        local: {
          ...
          items: [
            "日記の内容（タイトル・本文・日付・気分・天気）",
            "日記に添付した写真（端末内のアプリ専用フォルダに保存）",
            "アプリ設定（テーマカラー・ダークモード・自動保存設定など）",
            "作成中の日記の下書きデータ",
            "アプリロック用のパスコード",
          ],
        },
      ...
      retention: {
        title: "5. データの保持と削除",
        content: "すべてのデータ（日記・写真を含む）はお使いの端末内に保存されます。アプリをアンインストールすると、端末内に保存されたすべての日記データ・写真・設定情報が削除されます。削除されたデータの復元はできませんので、必要に応じて端末のバックアップ機能をご活用ください。",
      },
      permissions: {
        title: "6. アプリのアクセス権限",
        ...
        items: [
          {
            name: "カメラ",
            detail: "日記に添付する写真をその場で撮影するために使用します。撮影した写真は端末内にのみ保存され、外部に送信されることはありません。",
          },
          {
            name: "フォトライブラリ",
            detail: "端末内の既存の写真を日記に追加するために使用します。また、写真の編集（トリミング等）の際に、フォトライブラリへの保存が必要になる場合があります。",
          },
          {
      ...
      security: {
        title: "7. セキュリティ",
        content: "本アプリはネットワーク通信を行わないため、外部からの情報漏洩リスクは最小限です。日記データはお使いの端末内のデータベースおよびストレージに保存されており、端末のセキュリティ設定（パスコード・Face ID等）によって保護されます。",
      },
      children: {
        title: "8. お子様のプライバシー",
      ...
      userRights: {
        ...
        howTo: {
          title: "データの削除方法",
          content: "アプリ内で日記を個別に削除するか、アプリをアンインストールすることですべてのデータを削除できます。",
        },
      },
      changes: {
  ...
  ko: {
    ...
    sections: {
      intro: {
        title: "1. 소개",
        content: "Dairygram(이하 '본 앱')는 사진・기분・날씨를 곁들여 하루에 하나의 일기를 기록할 수 있는 포토 다이어리 앱입니다. 본 개인정보 처리방침은 본 앱 이용 시 정보 처리 방법에 대해 설명합니다.",
      ...
      dataCollection: {
        ...
        local: {
          ...
          items: [
            "일기 내용(제목・본문・날짜・기분・날씨)",
            "일기에 첨부한 사진(기기 내 앱 전용 폴더에 저장)",
            "앱 설정(테마 색상・다크 모드・자동 저장 설정 등)",
            "작성 중인 일기의 임시 저장 데이터",
            "앱 잠금용 패스코드",
          ],
        },
      ...
      purpose: {
        ...
        items: [
          "일기・사진・설정의 저장 및 표시",
          "캘린더 표시 및 기분・게시 수 통계에 반영",
          "작성 중인 일기의 임시 저장 및 복원",
      ...
      retention: {
        title: "5. 데이터 보관 및 삭제",
        content: "모든 데이터(일기・사진 포함)는 사용 중인 기기 내에 저장됩니다. 앱을 삭제하면 기기 내에 저장된 모든 일기 데이터・사진・설정 정보가 삭제됩니다. 삭제된 데이터는 복원할 수 없으므로, 필요에 따라 기기의 백업 기능을 활용해 주세요.",
      },
      permissions: {
        title: "6. 앱 접근 권한",
        ...
        items: [
          {
            name: "카메라",
            detail: "일기에 첨부할 사진을 그 자리에서 촬영하기 위해 사용합니다. 촬영한 사진은 기기 내에만 저장되며 외부로 전송되지 않습니다.",
          },
          {
            name: "포토 라이브러리",
            detail: "기기 내 기존 사진을 일기에 추가하기 위해 사용합니다. 또한 사진 편집(자르기 등) 시 포토 라이브러리에 저장이 필요한 경우가 있습니다.",
          },
          {
      ...
      security: {
        title: "7. 보안",
        content: "본 앱은 네트워크 통신을 수행하지 않으므로 외부로의 정보 유출 위험이 최소화되어 있습니다. 일기 데이터는 사용 중인 기기 내의 데이터베이스 및 저장소에 보관되며, 기기의 보안 설정(패스코드・Face ID 등)에 의해 보호됩니다.",
      },
      children: {
        title: "8. 아동의 개인정보",
      ...
      userRights: {
        ...
        howTo: {
          title: "데이터 삭제 방법",
          content: "앱 내에서 일기를 개별 삭제하거나 앱을 삭제함으로써 모든 데이터를 삭제할 수 있습니다.",
        },
      },
      changes: {
  ...
  en: {
    ...
    sections: {
      intro: {
        ...
        content: "Dairygram (hereinafter \"the App\") is a photo diary app that lets you record one entry per day along with photos, mood, and weather. This Privacy Policy explains how information is handled when you use the App.",
      ...
      dataCollection: {
        ...
        local: {
          ...
          items: [
            "Diary entry content (title, body text, date, mood, weather)",
            "Photos attached to diary entries (stored in an app-private folder on your device)",
            "App settings (theme color, dark mode, auto-save settings, etc.)",
            "Draft data for an entry in progress",
      ...
      purpose: {
        ...
        items: [
          "Saving and displaying diary entries, photos, and settings",
          "Reflecting entries in the calendar view and mood/post-count statistics",
          "Saving and restoring the draft of an entry in progress",
      ...
      retention: {
        ...
        content: "All data, including diary entries and photos, is stored on your device. Uninstalling the App will delete all diary data, photos, and settings stored on your device. Deleted data cannot be recovered, so please make use of your device's own backup feature if needed.",
      },
      permissions: {
        ...
        items: [
          {
            name: "Camera",
            detail: "Used to take a photo on the spot to attach to a diary entry. Photos taken are stored only on your device and are never transmitted externally.",
          },
          {
            name: "Photo Library",
            detail: "Used to add an existing photo from your device to a diary entry. Saving to the photo library may also be required when editing (e.g., cropping) a photo.",
          },
          {
      ...
      security: {
        title: "7. Security",
        content: "Since the App does not perform network communication, the risk of external data leakage is minimal. Diary data is stored in a database and storage on your device, and is protected by your device's security settings (passcode, Face ID, etc.).",
      },
      children: {
      ...
      userRights: {
        ...
        howTo: {
          ...
          content: "You can delete individual diary entries within the App, or uninstall the App to remove all data.",
        },
      },
      changes: {
...
// メインページ
// ========================================
export default function DairygramPrivacyPage() {
